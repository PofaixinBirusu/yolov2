...
def do_kmeans(n_anchors, boxes, centroids):
    ...
    for i in range(n_anchors):
        new_centroids[i].w /= len(groups[i])+1e-16
        new_centroids[i].h /= len(groups[i])+1e-16
...
def compute_centroids(boxes, n_anchors, loss_convergence, grid_size_w, grid_size_h, iterations_num, plus):
    ...
    while (True):
        ...
        for centroid in centroids:
            print(centroid.w * grid_size_w, centroid.h * grid_size_h)
